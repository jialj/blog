...
let queryList = params => {
  ...
  ON a.user_id = d.id
  WHERE 1=1 ` + (params.collect_id ? `AND a.collect_id = ${params.collect_id}` : "") + (params.id ? `AND a.id = ${params.id}` : "") + ` ORDER BY a.${params.orderBy} ${params.order}
...
let updateArticle = obj => {
  let setStr = (obj.type_id ? `type_id = ${obj.type_id},` : '') + (obj.title ? `title = '${obj.title}',` : '') + (obj.content ? `content = '${escape(obj.content)}',` : '') + (obj.create_time ? `create_time = '${obj.create_time}',` : '') + (obj.from ? `from = '${obj.from}',` : '') + (obj.author ? `author = '${obj.author}',` : '');
  if (setStr.length > 1) {
